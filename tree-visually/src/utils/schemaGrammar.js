...
export const grammarWrapperw = function () {
  function id(x) {
    return x[0];
  }
  ...
  var grammar = {
    Lexer: lexer,
    ParserRules: [
      { name: 'program', symbols: ['_ml', 'statements', '_ml'], postprocess: (data) => data[1] },
      ...
      {
        name: 'table_props$ebnf$1',
        symbols: ['table_props$ebnf$1', 'table_props$ebnf$1$subexpression$1'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      {
        name: 'table_props',
        symbols: ['table_prop', 'table_props$ebnf$1'],
        postprocess: (data) => {
          const repeated = data[1];
          const restStatement = repeated.map((chunk) => chunk[1]);
          return [data[0], ...restStatement];
      ...
      {
        name: 'table_prop$ebnf$1',
        symbols: ['table_prop$ebnf$1', 'table_prop$ebnf$1$subexpression$1'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      ...
      {
        name: '__lb_$ebnf$1',
        symbols: ['__lb_$ebnf$1', '__lb_$ebnf$1$subexpression$2'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      ...
      {
        name: '_ml$ebnf$1',
        symbols: ['_ml$ebnf$1', '_ml$ebnf$1$subexpression$1'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
      ...
      {
        name: '__ml$ebnf$1',
        symbols: ['__ml$ebnf$1', '__ml$ebnf$1$subexpression$2'],
        postprocess: function arrpush(d) {
          return d[0].concat([d[1]]);
        },
      },
...
export const grammarWrapper = function () {
  ...
  const comment = /--.*?$/;
  const keyword = ['ref:'];
  // identifiers could be eng/_/nums (starts only with non num)
  const identifier = /[a-zA-Z][a-zA-Z_0-9()]*/;
  const lParen = '(';
  const rParen = ')';
  const NL = { match: /\r?\n/, lineBreaks: true };
  ...
  var grammar = {
    ...
    {"name": "statement", "symbols": ["define"], "postprocess": id},
    {"name": "statement", "symbols": [(lexer.has("comment") ? {type: "comment"} : comment)], "postprocess": id},
    {"name": "define", "symbols": [{"literal":"NODE"}, "_", {"literal":"("}, "__lb_", "node_prop", "__lb_", {"literal":")"}], "postprocess": 
        data => data[4]
                },
    {"name": "node_prop", "symbols": [{"literal":"id"}, "_", (lexer.has("identifier") ? {type: "identifier"} : identifier), "__lb_", {"literal":"value"}, "_", (lexer.has("identifier") ? {type: "identifier"} : identifier), "__lb_", {"literal":"parent"}, "_", (lexer.has("identifier") ? {type: "identifier"} : identifier)], "postprocess": 
        data => ({
            "id"       : data[2],
            "value"    : data[6],
            "parent"   : data[10] 
