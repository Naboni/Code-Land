    ...
    return useQuery({
      ...
      queryFn: async () => {
        const { data } = await axios.get(
          `http://localhost:3001/favorite/user/${userId}`
        );
...
export default function Favorites() {
  ...
  const { status, data } = useFavorites(authUser?.uid);
      ...
      <div className={classes.questions}>
        <CenterContent>
          {status === 'loading' ? (
            ...
            <FormattedQuestions questions={data[0]?.questions} />
          )}
        </CenterContent>
      </div>
    </div>
