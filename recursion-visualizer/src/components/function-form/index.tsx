...
const FunctionForm = ({ onSubmit, onThemeChange }: Props) => {
  ...
  const [memoize, setMemoize] = useLocalStorageState('memoize', true)
  ...
  return (
    <s.FormContainer onSubmit={handleFormSubmit}>
      ...
      >
        <div>
          <s.Title>Famous Algorithms</s.Title>
          <s.Select
          ...
          <s.Title>Recursive function</s.Title>
            ...
            <CodeEditor
              ...
              shouldValueChange={(prevFnCode) => {
                return LanguageHandler.for(lang).validateFunctionCode(
                  prevFnCode
                )
              }}
              ...
              onValueReset={() => {
                const { fnCode } = LanguageHandler.for(
                  lang
                ).unparseFunctionData({
                  body: '',
                })
